...
@Component({
  selector: 'ngs-question-builder-container',
...
export class QuestionBuilderContainerComponent implements OnInit {
  ...
  constructor(
    ...
    this.builderOptionsSub = this._ngSurveyStore.builderOptions.subscribe(res => {
  ...
  handleCustomElementButton(element: IElements, cb): void {
    this._ngSurveyStore.optionAnswers.pipe(map(res => {
